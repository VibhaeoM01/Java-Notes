...
-- Q10 
select ei.employee_id,ei.employeename,di.deptname,si.basic from employee_info ei join dept_info di on ei.dept_id=ei.dept_id join salary_info si
on ei.employee_category=si.employee_category where di.deptname='HR';

-- Q11
select ei.EMPID as EMPID, ei.EMPNAME as EMPNAME, di.DEPTNAME as DEPTNAME, si.HOUSE_RENT_ALLOWANCE as HOUSE_RENT_ALLOWANCE from employee_info ei join salary_info si on ei.Employee_category=si.Employee_category 
join dept_info di on ei.DEPTID=di.DEPTID where di.location="Bangalore" or di.location="Cochin";
...
-- Q13
select c.First_Name as FIRST_NAME, c.Last_Name as LAST_NAME, a.Account_id as ACCOUNT_ID from account a join customer c on a.Customer_id=c.Customer_id where balance>=50000 order by c.First_Name;
...
-- Q23
select p.first_name as "FIRST NAME", p.contact as "CONTACT" from passenger p join boardingpass bp on p.passenger_id=bp.passenger_id join flight f on bp.flight_id=f.flight_id where f.flight_id=4 and f.flight_from='Hong Kong' and bp.meal='Vegetarian';
